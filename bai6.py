...
def train_model():
    ...
    X = df.iloc[:200, :5]  # Lấy cột từ 0 đến 4 làm đặc trưng
    y = df.iloc[:200, 5]  # Cột thứ 6 là nhãn (Performance Index)
...
def test_model():
    ...
    plt.ylabel("Giá trị")
    plt.show()
# Hàm huấn luyện và so sánh các mô hình
def compare_algorithms():
    ...
    # Vẽ biểu đồ so sánh các chỉ số MSE, RMSE, MAE
    plt.figure(figsize=(10, 6))

    # Biểu đồ cột cho MSE
    plt.subplot(1, 3, 1)
    plt.bar(results["Algorithm"], results["MSE"], color='lightblue')
    plt.title("MSE (Mean Squared Error)")
    plt.ylabel("MSE")

    # Biểu đồ cột cho RMSE
    plt.subplot(1, 3, 2)
    plt.bar(results["Algorithm"], results["RMSE"], color='lightgreen')
    plt.title("RMSE (Root Mean Squared Error)")

    # Biểu đồ cột cho MAE
    plt.subplot(1, 3, 3)
    plt.bar(results["Algorithm"], results["MAE"], color='lightcoral')
...
# Nút để so sánh các mô hình
tk.Button(root, text="Compare Algorithms", command=compare_algorithms).grid(row=2, column=0)
